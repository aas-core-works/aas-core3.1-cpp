...
#include <aas_core/aas_3_1/wstringification.hpp>
...
bool DetermineRecordMode() {
  const char* variable_name = "AAS_CORE_AAS_3_1_TEST_RECORD_MODE";
...
std::filesystem::path DetermineTestDataDir() {
  const char* variable_name = "AAS_CORE_AAS_3_1_TEST_DATA_DIR";
...
std::wstring TraceMark(
  const aas_core::aas_3_1::types::IClass& that
) {
  const std::wstring model_type = aas::wstringification::to_wstring(
    that.model_type()
  );

  if (aas_core::aas_3_1::types::IsIdentifiable(that)) {
  ...
  if (aas_core::aas_3_1::types::IsReferable(that)) {
  ...
  if (record_mode) {
    ...
    INFO(
      aas::common::Concat(
        ...
        "please set the environment variable AAS_CORE_AAS_3_1_TEST_RECORD_MODE "
        "to 'ON'"
      )
    )
