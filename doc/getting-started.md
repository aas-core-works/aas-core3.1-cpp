...
### Installation of Dependencies with VCPKG in Linux

Install the VCPKG in your `~/apps` directory:
...
The file `vcpkg` should be now available in the `~/apps/vcpkg` directory.

Change now to the repository root of our SDK.

Get the dependencies with vcpkg:

```
~/apps/vcpkg/vcpkg install --triplet x64-linux --x-install-root=$(pwd)/.vcpkg
```

The dependencies are downloaded and installed in `.vcpkg`.
...
Our SDK is exported as `aas_core3_1` to CMake.
You have to find it first in your `CMakeLists.txt`:

```
find_package(AAS_CORE3_1 0 REQUIRED)
```
...
target_link_libraries(some_cool_program aas_core3_1)
```
...
* **Classes**. For each class in the meta-model, we introduce two C++ classes. First, a purely virtual interface
  class (*e.g.,* @ref aas_core::aas_3_1::types::ISubmodel). Then, a concrete class (*e.g.,* @ref aas_core::aas_3_1::types::Submodel). All the pointers in the model use interfaces. We use concrete classes only for construction.
...
* **Optional values**. If you are compiling with C++17 and above, we represent optional values with `std::optional`.
  Otherwise, we rely on [tl::optional]. We re-export the one of the two which we use with `using` directive as   `aas_core::aas_3_1::common::optional`.
* **Aggregations**. We model the aggregations of the meta-model as [std::vector]'s in the C++.

### `IClass`

We introduce the most general class, @ref aas_core::aas_3_1::types::IClass, to group all the functionality of the model instances.
...
To alleviate that problem, each instance of the model is given an immutable getter of the runtime type as @ref aas_core::aas_3_1::types::IClass::model_type.
The model type will hold one of the concrete literals of the enumerator @ref aas_core::aas_3_1::types::ModelType, corresponding to the runtime type of model instance. 
...
When you need to cast among different model types, @ref aas_core::aas_3_1::types::IClass::model_type will still spare you a sequence of `if-else if-else if`'s.
...
See, for example, @ref aas_core::aas_3_1::types::IsSubmodel.
...
The namespace @ref aas_core::aas_3_1::types contains all the classes of the meta-model.
You can simply use their constructors to create an AAS model.
Usually you start bottom-up, all the way up to the @ref aas_core::aas_3_1::types::IEnvironment.
...
For example, see @ref aas_core::aas_3_1::types::ISubmodel::KindOrDefault.
...
The SDK provides two functions, @ref aas_core::aas_3_1::common::WstringToUtf8 and @ref aas_core::aas_3_1::common::Utf8ToWstring, to convert between UTF-8 and wide strings.
...
(We will alias the namespace @ref aas_core::aas_3_1 as `aas` for readability. You might or might not want to write your  code like that; the aliasing is not necessary.)
...
The SDK provides two modules, @ref aas_core::aas_3_1::stringification and @ref aas_core::aas_3_1::wstringification, to convert enumerators to and from strings and wide strings, respectively.

All enumerators are converted using the overloaded functions @ref aas_core::aas_3_1::stringification::to_string and @ref aas_core::aas_3_1::wstringification::to_wstring, respectively.

To parse an enumerator from a string to a literal, use `XxxFromString` and `XxxFromWstring`, respectively.
For example, see @ref aas_core::aas_3_1::stringification::ModelTypeFromString.

If you want to iterate over all literals of an enumerator, the SDK provides `kOverXxx` in @ref aas_core::aas_3_1::iteration.
For example, @ref aas_core::aas_3_1::iteration::kOverAasSubmodelElements.
...
The iteration code resides in @ref aas_core::aas_3_1::iteration.

### `Descent` and `DescentOnce`

The SDK provides two iterables, @ref aas_core::aas_3_1::iteration::Descent and @ref aas_core::aas_3_1::iteration::DescentOnce.
...
```cpp
#include <aas_core/aas_3_1/iteration.hpp>
...
**`IVisitor`**. The SDK provides an abstract, purely virtual, interface @ref aas_core::aas_3_1::visitation::IVisitor.
...
The abstract visitor @ref aas_core::aas_3_1::visitation::AbstractVisitor will take care of the children references.
You merely have to specify the operation for each model type.

**`PassThroughVisitor`**. If you want to process only some of the model types, use the class @ref aas_core::aas_3_1::visitation::PassThroughVisitor.
...
Here is an example with a @ref aas_core::aas_3_1::visitation::PassThroughVisitor, which has been adapted from the previous filtering example:

```cpp
#include <aas_core/aas_3_1/types.hpp>
#include <aas_core/aas_3_1/visitation.hpp>
...
The verification logic is concentrated in @ref aas_core::aas_3_1::verification.

Similar to descent, the verification is represented as an iterable over verification errors.
For a recursive verification over the instance and its references, use @ref aas_core::aas_3_1::verification::RecursiveVerification.
If you only want to check the instance and its immediate references, use @ref aas_core::aas_3_1::verification::NonRecursiveVerification.
...
```cpp
#include <aas_core/aas_3_1/types.hpp>
#include <aas_core/aas_3_1/verification.hpp>
...
int main() {
  ...
  // only feature letters, digits, hyphen (``-``) and  underscore (``_``); starting
...
The path to the erroneous value is kept in @ref aas_core::aas_3_1::iteration::Path, based on the enumeration @ref aas_core::aas_3_1::iteration::Property.
...
Our SDK handles the de/serialization of the AAS models from and to JSON format through the @ref aas_core::aas_3_1::jsonization.
...
The model instances are converted to [nlohmann/json] using @ref aas_core::aas_3_1::jsonization::Serialize.

Here is an example code:

```cpp
#include <aas_core/aas_3_1/jsonization.hpp>
#include <aas_core/aas_3_1/types.hpp>
...
For example, to de-serialize an environment, call @ref aas_core::aas_3_1::jsonization::EnvironmentFrom.

The de-serialization functions return a `aas_core::aas_3_1::common::expected`, a union type holding either the de-serialized instance or an error.

<a name="note-expected"></a>
@note @parblock 
As a matter of fact, we did not implement `aas_core::aas_3_1::common::expected` ourselves.
...
The code that de/serializes AAS models from and to XML documents lives in @ref aas_core::aas_3_1::xmlization.

### Serialization
You serialize a model using the overloaded functions @ref aas_core::aas_3_1::xmlization::Serialize.

Apart from the object, you pass it the writing options (@ref aas_core::aas_3_1::xmlization::WritingOptions) and the stream to write to.
...
```cpp
#include "aas_core/aas_3_1/types.hpp"
#include "aas_core/aas_3_1/xmlization.hpp"
...
For example, @ref aas_core::aas_3_1::xmlization::EnvironmentFrom.

The model instances are de-serialized from a text stream encoded in UTF-8.
There is an argument to steer the reading, @ref aas_core::aas_3_1::xmlization::ReadingOptions.
...
Similar to JSON, the result of the parsing is returned as `aas_core::aas_3_1::common::expected`.
...
The current library ignores it, and there is no easy way for you to find out to which @ref aas_core::aas_3_1::types::ISubmodel a particular @ref aas_core::aas_3_1::types::ISubmodelElement belongs to.
...
We therefore pre-generated the most of the boilerplate code in @ref aas_core::aas_3_1::enhancing.
...
The @ref aas_core::aas_3_1::enhancing is generic and can work with any form of enhancement classes.
You need to specify your enhancement factory as a [std::function] which takes an instance of @ref aas_core::aas_3_1::types::IClass as input and returns a [std::shared_ptr] to an enhancement, or `null`, if you do not want to enhance the particular instance.

[std::function]: https://en.cppreference.com/w/cpp/utility/functional/function

The wrapping and unwrapping is specified in the generic function @ref aas_core::aas_3_1::enhancing::Wrap.
...
You retrieve the enhancement with @ref aas_core::aas_3_1::enhancing::Unwrap from a model instance.
If the given instance has not been wrapped, `nullptr` is returned.

We also provide a shortcut function, @ref aas_core::aas_3_1::enhancing::MustUnwrap, which always return a pointer to the enhancement, but throws an exception if the instance has not been previously wrapped.
...
```cpp
#include "aas_core/aas_3_1/enhancing.hpp"
#include "aas_core/aas_3_1/types.hpp"
...
We demonstrate now how you can selectively enhance only some instances in the @ref aas_core::aas_3_1::types::IEnvironment.

For example, let us assign a unique identifier to all instances which are referable, *i.e.*, implement @ref aas_core::aas_3_1::types::IReferable.
