...
#include "aas_core/aas_3_1/constants.hpp"
#include "aas_core/aas_3_1/pattern.hpp"
#include "aas_core/aas_3_1/revm.hpp"
  ...
  while (true) {
    switch (state_) {
      ...
      case 2: {
        ...
        error_ = common::make_unique<Error>(
          L"Identifier shall have a maximum length of 2048 characters."
        );
        // No path is prepended as the error refers to the value itself.
        ++index_;
  ...
  while (true) {
    switch (state_) {
      ...
      case 2: {
        ...
        error_ = common::make_unique<Error>(
          L"Value type IEC 61360 shall have a maximum length of 2048 "
          L"characters."
        );
        // No path is prepended as the error refers to the value itself.
        ++index_;
  ...
  while (true) {
    switch (state_) {
      ...
      case 2: {
        ...
        error_ = common::make_unique<Error>(
          L"Content type shall have a maximum length of 128 characters."
        );
        // No path is prepended as the error refers to the value itself.
        ++index_;
    ...
    switch (state_) {
      ...
      case 3: {
        ...
        error_ = common::make_unique<Error>(
          ...
          L"hyphen (``-``) and  underscore (``_``); starting mandatory "
          L"with a letter, and not ending with a hyphen, *I.e.* "
          L"``^[a-zA-Z][a-zA-Z0-9_-]*[a-zA-Z0-9_]+$``."
        );
        // No path is prepended as the error refers to the value itself.
        ++index_;
...
void OfEntity::Execute() {
  while (true) {
    switch (state_) {
      ...
      case 13: {
        if (
          !(instance_->entity_type().has_value())
        ...
        error_ = common::make_unique<Error>(
          ...
          L"or specific asset ID must be set if Entity/entityType is "
          L"set to :attr:`Entity_type.Self_managed_entity`."
        );
        // No path is prepended as the error refers to the instance itself.
        ++index_;
